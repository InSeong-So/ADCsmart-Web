...
<div class="portlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all" id="${(widgetInfo.index)!''}">
	...
	<#if widgetInfo.type?? && (widgetInfo.type == 8 || widgetInfo.type == 9)>
		<#if widgetTarget.category == 2>
			ADC: 
		<#elseif widgetTarget.category == 3 || widgetTarget.category == 4>
			VIP:
		</#if>	
	</#if>
	
	<span class="sub_title">${(widgetTarget.name)!''}</span>
		<#if widgetTarget.category == 7 || widgetTarget.category == 8>	
	...
	<div class="portlet-content-chart" style="margin-top: 0px; padding-top: 0px;">		
	</div>
</div>