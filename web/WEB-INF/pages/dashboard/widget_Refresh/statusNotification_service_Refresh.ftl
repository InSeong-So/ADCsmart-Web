...
<div class="portlet-header ui-widget-header ui-corner-all">${(widgetInfo.name)!''} -
	...
	<div class="portlet-content">
		...
		<#if notifcationList??>
			...
			<#if (notification.targetIp)??>
			<div class="status_notification" id="${notification.targetIndex!''}" idx="${notification.targetIp!""}">
				<a class="serviceMonitorLnk" title="" href="">
					<#if notification.status == 1>
						<span class="show vs_conn"></span>
					<#elseif notification.status == 2>
						<span class="show vs_caution"></span>
					<#elseif notification.status == 0>
						<span class="show vs_disconn"></span>
					<#else>
					</#if>
					<span title="${notification.targetName!''}&nbsp;&#13;${notification.targetIp!""}">
						<p class="vname none">${notification.targetName!''}</p>
