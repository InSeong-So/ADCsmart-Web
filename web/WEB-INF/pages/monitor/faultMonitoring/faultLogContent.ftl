...
<div class="contents_area">
	<div>
		<#if adcObject.category == 0>
			<img src="imgs/title${img_lang!""}/h3_adcfaultLog_all.gif" class="title_h3" />
		<#elseif adcObject.category == 1>
			<img src="imgs/title${img_lang!""}/h3_adcfaultLog_group.gif" class="title_h3" />
		<#elseif adcObject.category == 2>
			<img src="imgs/title${img_lang!""}/h3_adcfaultLog_adc.gif" class="title_h3" />
		<#else>
		</#if>
	...
	<div class="control_Board">
        ...
        <div class="control_positionR">
        	<input type="button" class="Btn_white s-exportbtn" value="${LANGCODEMAP["MSG_ADCLOG_OUT_FILE"]!}"/>         	
        	<input type="button" class="Btn_white s-refreshbtn" value="${LANGCODEMAP["MSG_ADCLOG_REFRESH"]!}"/>         	         	
		</div>
	</div>
	<br class="clearfix" />
	<table class="Board faultLogTable" cellpadding="0" cellspacing="0"  style="table-layout: fixed;">
		<#if adcObject.category == 2>
			<#include "faultTableInListContent.ftl"/>
		<#else>
			<#include "faultTableInListGroupContent.ftl"/>
		</#if>			
	</table>
...
<!-- 상세장애로그팝업  시작-->
<div id="faultlog-popup" class="pop_type_wrap">
    <h2>${LANGCODEMAP["MSG_FAULT_DETAIL"]!}</h2>
    <div class="pop2_contents">        
        <div class="description condition">
            <table class="Board_100" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <th class="Lth2">${LANGCODEMAP["MSG_FAULT_ADC_NAME"]!}</th>
                    <td class="Lth0 adcname-area">-</td>
                    <th class="Lth2">${LANGCODEMAP["MSG_FAULT_OCCU_TIME"]!}</th>
                ...
                <tr>
                    <th class="Lth2">${LANGCODEMAP["MSG_FAULT_TYPE"]!}</th>
                    <td class="Lth0 type-area" colspan="3">-</td>                                                        
                </tr>            
                <tr class="DivideLine">
                    <td colspan="4"></td>
                </tr>                
        ...
        <div class="position_cT10">
 			<input type="button" class="popupclosebtn Btn_white" value="${LANGCODEMAP["MSG_MONITOR_ALERT_CLOSE"]!}"/>        
    </div>
    <p class="close">
