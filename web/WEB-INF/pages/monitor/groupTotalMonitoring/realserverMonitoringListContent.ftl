...
<div class="contents_area">
	<div>
		<#if adcScope.level == 0>
			<img src="imgs/title${img_lang!""}/h3_monitor_realserver_all.gif" class="title_h3" />
		<#elseif adcScope.level == 1>
			<img src="imgs/title${img_lang!""}/h3_monitor_realserver_group.gif" class="title_h3" />
		<#else>
			<img src="imgs/title${img_lang!""}/h3_monitor_realserver_adc.gif" class="title_h3" />
		</#if>	
...
<div class="control_Board">
	...
	<table class="Filter_Board optionFilter none" cellpadding="0" cellspacing="0"  style="table-layout: fixed;">
		<thead>
			...
			<#if montotalRs.condition.status.isSelect()>
				<th data-colidx="0">${LANGCODEMAP["MSG_GROUPMONITOR_STATUS"]!}</th>
			<#else>
				<th class="none" data-colidx="0">${LANGCODEMAP["MSG_GROUPMONITOR_STATUS"]!}</th>
			</#if>
			<#if montotalRs.condition.state.isSelect()>
				<th data-colidx="1">State</th>
			<#else>
				<th class="none" data-colidx="1">State</th>
			</#if>
			<#if montotalRs.condition.used.isSelect()>
				<th data-colidx="4">${LANGCODEMAP["MSG_GROUPMONITOR_USEFLAG"]!}</th>
			<#else>
				<th class="none" data-colidx="4">${LANGCODEMAP["MSG_GROUPMONITOR_USEFLAG"]!}</th>
			</#if>		
			</tr>
		</thead>
		<tbody>
			...
			<#else>
				<td class="none" data-colidx="0">
					...
					<ul class="chklist" data-group="group1" data-colidx="0">
						<#if montotalRs.condition.status.isSelect()>
							<#list montotalRs.condition.status.filter as theFilter>
			...
			<#else>
				<td class="none" data-colidx="1">
					...
					<ul class="chklist" data-group="group2" data-colidx="1">	
						<#list montotalRs.condition.state.filter as theFilter>
						<#if (theFilter.index == 0)>
						<li class="chkFilterAll none">	
							<#if theFilter.isSelect()>		  				
				  				<input class="" name="selChk" type="checkbox" value="${(theFilter.value)!''}" checked="checked">
			...
			<#if montotalRs.condition.backup.isSelect()>
				<td data-colidx="4">
			<#else>
				<td class="none" data-colidx="4">
			</#if>	
					<div class="multcheck">
					<ul class="chklist"  data-group="group3" data-colidx="4">								
						<#list montotalRs.condition.used.filter as theFilter>
						<#if (theFilter.index == 0)>
						<li class="chkFilterAll none">	
							<#if theFilter.isSelect()>		  				
				  				<input class="" name="selChk" type="checkbox" value="${(theFilter.value)!''}" checked="checked">
	...
	<table id="realserverListTable" class="fixed-table_xScroll serviceMonListTable" cellpadding="0" cellspacing="0"  style="table-layout: fixed;">
		<#include "realserverMonitoringTableInListContent.ftl"/>
	</table>
	</div>
	</div>
