<table class="Atype connectTest" cellpadding="0" cellspacing="0">
	...
	<tr>
		...
		<#list chkResultList as thsChkResult>
			...
			<#elseif thsChkResult.checkID == 4>	
				<#if adcType == "Alteon">		
					<#if thsChkResult.status == 1>
						<#assign loginStatus = "o">					
						<td class="State_${networkStatus}x${loginStatus}"></td>
						<td class="State_ok loginOk">③ ${LANGCODEMAP["MSG_ADCSETTING_CHECK_NETWORK"]!}(ADC→ADCsmart)</td>			
					<#elseif thsChkResult.status == 2>	
						<#assign loginStatus = "f">		
						<td class="State_${networkStatus}x${loginStatus}"></td>
						<td class="State_fail loginFail">③ ${LANGCODEMAP["MSG_ADCSETTING_CHECK_NETWORK"]!}(ADC→ADCsmart)</td>
					<#else>		
						<td class="State_${loginStatus}xn"></td>
