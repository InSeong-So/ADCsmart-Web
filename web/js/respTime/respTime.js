var RespTime = Class.create({
	...
	{		
		...
		{			
			...
			ajaxManager.runHtmlExt({
				url : 'respTime/loadResponseTimeContent.action',
				data : 
				{
				},
	...
	loadRespGroupInfoList : function(index, ip)
	{
		...
		{		
			ajaxManager.runHtmlExt({
				url		: "respTime/loadResponseInfoList.action",
				data	: 
				{
...
//			};		
			...
			ajaxManager.runJsonExt({
				url			: "respTime/loadResponseTimeHistoryInfo.action",
				data		: params,
				successFn	: function(data)
				{		
...
//				if (!_validationFilterChk())
				...
				$(this).ajaxSubmit({
					dataType : 'json',
					url : isModify ? 'respTime/setRespIntervalCheck.action' : 'respTime/addRespIntervalCheck.action',
					data :
					{
...
//		{			
//			ajaxManager.runHtmlExt({
//				url : "respTime/loadRespInfoAddContent.action",
//				data :
//				{
//					"respIndex" : 0
//				},
...
//	},
	loadRespInfoModifyContent : function(isModify, index)
	{
		...
		{			
			ajaxManager.runHtmlExt({
				url : "respTime/loadRespInfoModifyContent.action",
				data :
				{
					"respIndex" : index
				},
...
//					registerEvents(isModify);
	...
	delRespInfos : function()
	{
		with (this)
		{
			...
			ajaxManager.runHtmlExt({
				url : "respTime/delRespIntervalCheck.action",
				data : 
				{
