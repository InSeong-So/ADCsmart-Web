var GroupMonitor = Class.create({
	initialize : function()
	{
		...
		{
			fn.loadGroupMonitoringTableInListContent(fn.searchedKey, fromRow, toRow, orderType, orderDir, fn.changeFilter(), fn.selectedCol());
		});
		this.target = {};
...
//			console.log('1111');
			...
			ajaxManager.runJsonExt({
				url : "monitor/retrieveGroupMonTotal.action",
				data : 
				{
				...
				successFn : function(data)
				{
					...
					loadGroupMonitorList(searchKey);
				},
				errorFn : function(jqXhr)
				{
					$.obAlertAjaxError(VAR_GROUP_LAODFAIL, jqXhr);
				}
			});
		}
	},
	loadGroupMonitorList : function(searchKey, fromRow, toRow)
	{
		with(this)
		{
			ajaxManager.runHtmlExt({
				url : "monitor/loadGroupMonitorList.action",
				data : 
				{
					"adcScope.level" 	: target.level,					
					"adcScope.index"	: target.index,
				...
				{
					header.setActiveMenu("MonitorGroup");					
					searchedKey = searchKey;
					
					if(groupMonitor.columnSet)
					{
						$.each(groupMonitor.columnSet, function(index, value)
						{
							showHidenCol($('#groupListTable'), index, value);
						});
					}
					
					if(groupMonitor.selectedSet)
					{
						var isChecked;
						$.each(groupMonitor.selectedSet, function(groupIndex, groupSet) {
						...
						var isFlag= groupMonitor.setFlag;
						if (isFlag == "false")
						{
						...
						loadGroupRefreshMonListContent(searchKey, orderDir, orderType);
					}
	...
	loadGroupRefreshMonListContent : function(searchKey, orderDir, orderType)
	{
		with(this)
		{
			...
			ajaxManager.runJsonExt({
				...
				successFn : function(data)
				{
					...
					pageNavi.updateRowTotal(rowTotal, orderType);
					loadGroupMonitoringTableInListContent(searchKey, undefined, undefined, undefined, undefined, filterData, selcolData);
				},
				errorFn : function(jqXhr)
				{
					$.obAlertAjaxError(VAR_GROUP_LAODFAIL, jqXhr);
	...
	loadGroupMonitoringTableInListContent : function(searchKey, fromRow, toRow, orderDir, orderType, filterData, selcolData)
	{
		with(this)
		{
			ajaxManager.runJsonExt({
				url : "monitor/loadGroupMonitoringTableInListContent.action",
				data : 
				{
					"adcScope.level" 	: target.level,					
					"adcScope.index"	: target.index,
...
//					console.log(data);
					...
					refreshHeaderSort($('#groupListTable'), orderType, orderDir); 
	...
	{
		var rowTemplate = $('#groupListTable tbody .ContentsLine1.none').clone().removeClass('none');
//		var divideLineTemplate = '<tr class="DivideLine"><td colspan="8"></td></tr>';
//		var endLineTemplate = '<tr class="EndLine"><td colspan="8"></td></tr>';
//		rowTemplate.find('td').text('');
		
		$('#groupListTable tbody tr:not(.none)').remove();
		//console.log(rowTemplate, data);
		var row;
//		row = rowTemplate.clone();
		$.each(data.montotalGrp.groupList, function(index, rowData)
		{
			row = rowTemplate.clone();
			$.each(rowData, function(key, value)
			{
				...
				else if(key == "member" || key == "filter" || key == "vsAssigned")
				{
					if(value == -1)
					{
						html += '-';
...
//				else if(key == "member")
//				{
//					html += '<a href="javascript:;" class="realServerLnk"><span style="display:none;">' + rowData.adcIndex + '</span>' + value + '</a>';
//					row.find('td.' + key).empty().append(html);
//				}
//				else if(key == "filter" || key == "vsAssigned")
//				{
//					html += value + '건';
//					row.find('td.' + key).empty().append(html);
//				}
			...
			$('#groupListTable tbody:first').append(row);
			
//			$('#groupListTable tbody:first').append(divideLineTemplate);
		});
		
//		$('#groupListTable tbody').append(endLineTemplate);		
	},
...
//		$('.Filter_Board .chkFilterOption td:not(.none) .chkFilter input[type=checkbox]:checked').each(function(index) {
		$('.Filter_Board tbody tr:not(.none) .chkFilter input[type=checkbox]:checked').each(function(index) {
	...
	registerEvents : function()
	{
		with(this)
		{
			$('input[name="searchKey"]').attr("placeholder", VAR_GROUP_NAME);
			...
			$('#exportLnk').click(function(e)
			{
				e.preventDefault();
				var searchKey = $('input[name="searchKey"]').val();
				_checkExportGroupMonDataExist(searchKey);
			});
...
//				loadListContent();
				...
				loadGroupRefreshMonListContent(null, orderDir, orderType);	
			});	
			...
			$('.selectFilter').on('click', '.deselBtn', function(e)
			{
				...
				loadGroupRefreshMonListContent($('input[name="searchKey"]').val(), "", "", changeFilter(), selectedCol());		
			});
			...
			{
				e.preventDefault();		
				groupMonitor._loadSelColPop();			
			});
			
			//link			
			$('#groupListTable').on('click', '.serviceMapLnk', function(e)
			{
				e.preventDefault();
				with (this) 
				{
			...
			$('#groupListTable').on('click', '.adcMonitoringLnk', function(e)
			{			
				e.preventDefault();
				with (this) 
				{
			...
			$('#groupListTable').on('click', '.realServerLnk', function(e)
			{
				e.preventDefault();
				with (this) 
				{			
					var adcIndex = $(this).children('span').text();
					adcSetting._selectAdc(adcIndex);
					realServerMonitor.loadListContent("", adcSetting.getAdc());
				}				
			});	
		}
	},
	...
	changeFilter : function()
	{
		with(this)
		{
			...
			groupMonitor.selectedSet = filterData;
			
			if ($('.chkFilter').find('input:checked').length > 0)				
				groupMonitor.setFlag = $('#filterAdd').attr("isFlag");
	...
	selectedCol : function()
	{
		...
		{
			var selColData = [];
			$('#groupListTable .ContentsHeadLine th').each(function()
			{
	...
	_loadSelColPop : function()
	{
		with(this)
		{
			...
			$('#groupListTable .ContentsHeadLine').find('th').each(function()			
			{
...
//				}
			...
			$('.cloneDiv .onOk').click(function(e)
			{
				...
				{
					if ($(this).is(':checked'))
						showHidenCol($('#groupListTable'), index, true);
					else
						showHidenCol($('#groupListTable'), index, false);
				});
				
				groupMonitor.columnSet = selectedCol();
				groupMonitor.selectedSet = changeFilter();
...
//			}
	...
	_checkExportGroupMonDataExist : function(searchKey)
	{
		with(this)
		{
			var filterData = changeFilter();
			var selColData = selectedCol();
			ajaxManager.runJsonExt({
				url : "monitor/checkExportGroupMonDataExist.action",				
				data : 
				{
					"adcScope.level" 	: target.level,					
					"adcScope.index"	: target.index,
	...
	exportLnk : function()
	{
		with (this)
		{
			...
			var url = "monitor/downloadGroupMonList.action?" + params;
