var MemberAddBatchWnd =  Class.create({
	initialize : function() {
	},
	popUp : function() {
		with (this) {
	...
	_registerEvents : function() {
		with (this) {
			$('.cloneDiv .onOk').click(function(e) {
				...
				virtualServer.f5Vs.addIpAndPortsToMemberTable(ipAndPorts);
	...
	_validate : function(ips) {
		for (var i=0; i < ips.length; i++) {
			...
			if (ipAndPort.length < 2 || !getValidateIP(ipAndPort[0]) || !getValidateNumberRange(ipAndPort[1], 0, 65535))
				return false;
		}
