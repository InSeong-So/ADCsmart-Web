var GroupAlert = Class.create({
	initialize : function() 
	{		
		this.configLevel = undefined;
		this.target = {};
		this.adcTargetVendor = 2;
	},
	setTarget : function(GroupIndex)
	{
		if(!GroupIndex)
		{
			this.target.categoryIndex = 0;
			this.target.groupIndex = undefined;
		}
		...
		else
		{
			...
			this.target.groupIndex = GroupIndex;
		}
	},
	setTargetVendor : function(vendor) 
	{
		this.adcTargetVendor = vendor;
	},
	loadGroupAlertListContent : function() 
	{
		with (this) 
		{
			setTarget(adcSetting.getGroupIndex());
			ajaxManager.runHtmlExt({
				url : "adc/alert/loadGroupAlertListContent.action",
				data : 
				{
					"adcObject.category" : target.categoryIndex,
					"adcObject.index" : target.groupIndex,
					"adcObject.vendor" : 2					
				},
				...
				successFn : function(params) 
				{
					...
					registerGroupAlertListContentEvents();
					registerGroupAlertMainEvents();
					chkBoxControl();
				},
	...
	loadAlertListTargetChangeContent : function(Vendor) 
	{
		with (this) 
		{			
			ajaxManager.runHtmlExt({
				url : "adc/alert/loadAlertListTargetChangeContent.action",
				data : 
				{
					"adcObject.category" : target.categoryIndex,
					"adcObject.index" : target.groupIndex,
					"adcObject.vendor" : Vendor					
				},
				target: ".contents_area .alertTableArea",
				successFn : function(params) 
				{										
					registerGroupAlertListContentEvents();
					chkBoxControl();
		...
		{			
			ajaxManager.runJsonExt({
				...
				data : 
				{
					"adcObject.category" : target.categoryIndex,
					"adcObject.index" : target.groupIndex,
					"adcObject.vendor" : adcTargetVendor
				},				
				successFn : function(data) 
				{
					if(data.isSuccessful) 
					{						
						loadGroupAlertListContent();
					} 
					else 
					{	
	...
	registerGroupAlertMainEvents : function()
	{
		with (this)
		{
				...
				$(this).ajaxSubmit({
					...
					success : function(data)
					{
						if(data.isSuccessful) 
						{
							...
							loadAlertListTargetChangeContent(adcTargetVendor);
						} 
						else 
						{	
							$.obAlertNotice(data.message);
			...
			$('#targetTap_Alteon').click(function() 
			{
				TapClear();
				$(this).css({'background-color' : '#666666','color': '#fff'});				
				var adcTargetVendor = 2;
				setTargetVendor(adcTargetVendor);
				loadAlertListTargetChangeContent(adcTargetVendor);
			});
			$('#targetTap_F5').click(function() 
			{
				TapClear();
				$(this).css({'background-color' : '#666666','color': '#fff'});				
				var adcTargetVendor = 1;
				setTargetVendor(adcTargetVendor);
				loadAlertListTargetChangeContent(adcTargetVendor);
			});
			$('#targetTap_PAS').click(function() 
			{
				TapClear();
				$(this).css({'background-color' : '#666666','color': '#fff'});				
				var adcTargetVendor = 3;
				setTargetVendor(adcTargetVendor);
				loadAlertListTargetChangeContent(adcTargetVendor);
			});
			$('#targetTap_PASK').click(function() 
			{
				TapClear();
				$(this).css({'background-color' : '#666666','color': '#fff'});				
				var adcTargetVendor = 4;
				setTargetVendor(adcTargetVendor);
				loadAlertListTargetChangeContent(adcTargetVendor);
			});
		}
	...
	registerGroupAlertListContentEvents : function() 
	{
		with (this) 
		{
			...
			{			
				...
				{										
					...
					$('input[name="alarmConfigs.redundancyCheckAction.sms"]').val("1");
				}
				else 
				{					
			...
			$('.allSMSChk3').click(function(e) 
			{
				...
				{							
					...
					$('input[name="alarmConfigs.cpuFanBadAction.sms"]').val("0");
	...
	TapClear : function()
	{
		with(this)
		{
			$('#targetTap_Alteon').css({'background-color': '#fff', 'color': 'black'});
			$('#targetTap_F5').css({'background-color': '#fff', 'color': 'black'});
			$('#targetTap_PAS').css({'background-color': '#fff', 'color': 'black'});
			$('#targetTap_PASK').css({'background-color': '#fff', 'color': 'black'});					
		}
	},
