...
public class OBCipherAES {
	 ...
	 * <p>
	 * Contents are reset on each major call (setKey, encrypt, decrypt) and should
	 * be used after each of these calls returns for display.
	 */
...
//		try
//		{
//			String pass="!wkrdjq00";
//			String encrypt=new OBCipherAES().Encrypt(pass);
//			System.out.println(encrypt);
//			String decrypt=new OBCipherAES().Decrypt(encrypt);
...
//	}
	...
	public String Encrypt(String data) throws OBException {
		...
		try {
			...
			throw new OBException(OBException.ERRCODE_SYSTEM_DATAENCRYPT);
		}
	}

	public String Decrypt(String data)// throws OBException
	{
		if (data == null || data.isEmpty())
			return null;

		byte[] input = hexToByteArray(data);
		byte[] keyBytes = new byte[] { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c,
				0x0d, 0x0e, 0x0f };// , 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17 };

		SecretKeySpec key = new SecretKeySpec(keyBytes, "AES");

//		OBSystemLog.debug(OBDefine.LOGFILE_DEBUG, String.format("start. %s", data));	    	
		Cipher cipher;
		try {
			cipher = Cipher.getInstance("AES");// , "BC");
			// encryption pass
			...
			result = result.substring(0, ctLength);
//			OBSystemLog.debug(OBDefine.LOGFILE_DEBUG, String.format("end. %s/%d", result, ctLength));	    	
			return result;
		} catch (Exception e) {
			e.printStackTrace();
			OBSystemLog.error(OBDefine.LOGFILE_SYSTEM,
					String.format("failed to decrypt data. data:%s, message:%s", data, e.getMessage()));
