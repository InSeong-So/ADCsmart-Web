 ...
 */
public class OBCLIParserAlteon {
	private final static String SUFFIX_MORE = "any other key to continue[0m";
//	
//    final static int PARSE_STATE_INIT 		= 0;
//    final static int PARSE_STATE_SLB 		= 1;
//    final static int PARSE_STATE_REAL 		= 2;
//    final static int PARSE_STATE_HEALTH 	= 3;
//    
//    final static String SUFFIX_START_HEALTH = "health ";
//    final static String SUFFIX_END_HEALTH	= "apply";
//    
//    final static String SUFFIX_START_REAL 	= "real ";
//    final static String SUFFIX_END_REAL 	= "apply";
//    
//    final static String SUFFIX_START_VSRV 	= "slb ";
//    final static String SUFFIX_END_VSRV 	= "apply";
//    final static String SUFFIX_START_SERVICE= "! Define SLB service";
//    final static String SUFFIX_END_SERVICE 	= "..";

	private static final String STRING_TO_PROTOCOL_TCP = "tcp";
	private static final String STRING_TO_PROTOCOL_UDP = "udp";
	private static final String STRING_TO_PROTOCOL_ICMP = "icmp";

	private static final int INT_TO_PROTOCOL_TCP = 6;
	private static final int INT_TO_PROTOCOL_UDP = 17;
	private static final int INT_TO_PROTOCOL_ICMP = 1;

	public OBCLIParserAlteon() {
	}
...
//			System.out.println(new OBCLIParserAlteon().parseGeneralInfo(cfgDump));
//			handler.disconnect();
...
//	}
	...
	private final static String SUFFIX_GENERAL_IPADDR = ") Address: ";// MAC Address: 00:13:0a:51:bc:00 IP (If 1)
																		// Address: 192.168.100.11
	private final static String SUFFIX_GENERAL_SERIALNUM = "Serial No: ";// Hardware Order No: EB1412004 Serial No:
																			// SSCPC80159 Rev: 09
...
//	        private final static String SUFFIX_GENERAL_SWVERSIONE  	= "Software Version ";//Software Version 25.3.0 (FLASH image1), active configuration.
//	        private final static String SUFFIX_GENERAL_MADADDR  	= "MAC Address: ";//MAC Address: 00:13:0a:51:bc:00    IP (If 1) Address: 192.168.100.11
//	        private final static String SUFFIX_GENERAL_IPADDR 	 	= ") Address: ";//MAC Address: 00:13:0a:51:bc:00    IP (If 1) Address: 192.168.100.11
//	        private final static String SUFFIX_GENERAL_SERIALNUM 	= "Serial No: ";//Hardware Order No:   EB1412004     Serial No: SSCPC80159 Rev: 09
...
//>> scard_alt_slb1 - Layer 2#
	...
	public ArrayList<OBDtoInfoVlanAlteon> parseInfoVlan(String input) throws OBException {
		...
		for (String line : lines) {
			...
			// name 검출.
			item = line.substring(6, 38);
			obj.setName(item);
			// status 검출.
			item = line.substring(39, 44);
...
//		try
//		{
//			new OBCLIParserAlteon().zTestParseSeesionDumpList();
...
//			handler.login();
//			System.out.println("login OK");
//			String cfgDump = handler.cmndDumpInfoFdb();
//			System.out.println(new OBCLIParserAlteon().parseFdbDump(cfgDump));
//			handler.disconnect();
//			System.out.println("logout OK");
//		}
//		catch(Exception e)
...
//	
	/*
	 * 
	 * >> 5412_28.1.11_#1 - Standalone ADC - Main# /info/l2/fdb/dump MAC address
	 * VLAN Port Trunk State Age Referenced SPs Learned port Referenced ports
	 * ----------------- ---- ------ ----- ----- --- -------------- ------------
	 * ---------------- 00:00:5e:00:01:0a 1 1 FWD 1 2 1 1 00:00:5e:00:01:26 1 VSR P
	 * 1-7 empty 00:00:5e:00:01:34 1 VSR P 1-7 empty 00:00:5e:00:01:35 1 VSR P 1-7
	 * empty 00:00:5e:00:01:36 1 VSR P 1-7 empty 00:00:5e:00:01:37 1 1 FWD 1 2 1 1
	 */
	private final static String SUFFIX_FDB_DUMP_START = "----------------- ---- ------ ----- -----";//
	private final static String SUFFIX_FDB_DUMP_END = ">>";//

	public HashMap<String, OBDtoFdbInfo> parseFdbDump(String input) throws OBException {// key: mac addr
		HashMap<String, OBDtoFdbInfo> retVal = new HashMap<String, OBDtoFdbInfo>();
		boolean startFlag = false;
		String lines[] = input.split("\n");
		for (String line : lines) {
			if (line.isEmpty())
				continue;
			if (startFlag == false) {
				Integer strIndex = line.indexOf(SUFFIX_FDB_DUMP_START);
				if (strIndex >= 0) {
					startFlag = true;
				}
				continue;
			}
			Integer strIndex = line.indexOf(SUFFIX_FDB_DUMP_END);
			if (strIndex >= 0) {
				return retVal;
			}
			...
			// 첫번째 요소: MAC 주소.
			obj.setMacAddr(element[0].toLowerCase());

			// 두번째 요소: valn 번호.
			if (element.length >= 2)
				obj.setVlanID(Integer.parseInt(element[1]));

			retVal.put(element[0], obj);
		}
		return retVal;
	}
...
//>> scard_alt_slb1 - NTP Server# 
	...
	public ArrayList<OBDtoVSvcStatusAlteon> parseInfoSlbDump(String input) throws OBException {
		...
		for (String line : lines) {
			...
			if (line.matches(
					"\\d+\\:\\s+(IP4\\s+)?(\\d{1,3}\\.){3}\\d{1,3},\\s+([0-9a-fA-F]{2}\\:){5}[0-9a-fA-F]{2}.*") == true) {
				if (vsObj != null) {
					retVal.add(vsObj);
				}
...
//	    		String oneLine = line.substring(strIndex+SUFFIX_SLBDUMP_VSINFO.length());
			...
			if (realStartFlag == false) {
				if ((strIndex = line.indexOf(SUFFIX_SLBDUMP_REALSTART)) >= 0) { // 사용하지 않는 정보, 분석하지 않는다.
					realStartFlag = true;
					continue;
				}
			}
			...
			if (realStartFlag == true) // real server
			{
				...
				if (commaItems.length != 5) {
					OBSystemLog.info(OBDefine.LOGFILE_SYSTEM,
							String.format("failed to parse realserver's status(%s)", line));
					continue;
				}
...
//	}	
	 ...
	 * @throws OBException
	 */
	private final static String SUFFIX_INFO_SWKEY = "Throughput ";//
		...
		try {
			...
			for (String line : lines) {
				...
				if (strIndex >= 0) {
					...
					if (element.length <= 0) {
						OBSystemLog.error(OBDefine.LOGFILE_SYSTEM,
								String.format("failed to parse boot time(%s)", line));
						continue;
					}
...
//			handler.login();
//			System.out.println("login OK");
//			String cfgDump = handler.cmndInfoSessCip("172.172.2.209");//
//			System.out.println(new OBCLIParserAlteon().parseSeesionDumpList(1,cfgDump));
//			handler.disconnect();
//			System.out.println("logout OK");
//		}
//		catch(Exception e)
...
//   
	...
	public ArrayList<OBDtoFaultSessionInfo> parseSeesionDumpList(Integer adcIndex, String input, int resultLimit,
			String lbType) throws OBException {
		// 1,04: 172.172.2.110 43926, 192.168.100.51 ftp -> 7909 192.168.199.52 ftp age
		// 6 v:1 E
		Pattern exprMain = Pattern.compile("(\\d+),\\d+\\:\\s+([^\\s]+)\\s+(\\d+),\\s+(.+)"); // sp, src ip, src, port,
																								// dst part 식별

		// 1,04: 172.172.2.110 43926, 192.168.100.51 ftp -> 7909 192.168.199.52 ftp age
		// 6 v:1 E ---- DAM enable일때
		Pattern exprSlbDstDam = Pattern.compile(
				"([^\\s]+)\\s+([^\\s]+)\\s+->\\s+(\\d+)\\s+([^\\s]+)\\s+([^\\s]+)\\s+(\\w+\\s)?age\\s+(\\d+).*");
		// 1,04: 172.172.2.110 43926, 192.168.100.51 ftp -> 192.168.199.52 ftp age 6 v:1
		// E ---- DAM 아닐때
		Pattern exprSlbDstNotDamOrFilterWcr = Pattern
				.compile("([^\\s]+)\\s+([^\\s]+)\\s+->\\s+([^\\s]+)\\s+([^\\s]+)\\s+(\\w+\\s)?age\\s+(\\d+)\\s+v\\:.*");

		// 7,03: 70.70.70.100 1, 80.80.80.100 13701 DENY age 0 f:5 v:1
		// 7,03: 70.70.70.100 2049, 70.70.70.150 62310 ALLOW age 0 f:70 v:1
		// 3,00: 70.70.70.150 1, 80.80.80.100 2037 NAT age 0 f:5 v:1
		// allow, deny, nat는 포맷이 같아서 한번에 식별 가능
		Pattern exprFilterAllowDenyNat = Pattern
				.compile("([^\\s]+)\\s+([^\\s]+)\\s+(ALLOW|DENY|NAT)\\s+(\\w+\\s)?age\\s+(\\d+).*");
		// 30,05: 172.172.2.91 24039, 192.168.100.52 http -> 192.168.199.41 wcr tcp age
		// 2 f:1001 v:1 E
		Pattern exprFilterWcr = Pattern
				.compile("([^\\s]+)\\s+([^\\s]+)\\s+->\\s+([^\\s]+)\\s+([^\\s]+)\\s+(\\w+\\s)?age\\s+(\\d+)\\s+f\\:.*");
		...
		for (String line : lines) {
			...
			// 메인 파트를 확인한다.
			if ((matcher = exprMain.matcher(line)).matches() == true) {
				sp = matcher.group(1); // SP
				sourceIP = matcher.group(2);
				sourcePort = matcher.group(3);
				dstPart = matcher.group(4);

				if ((matcher = exprSlbDstDam.matcher(dstPart)).matches() == true
						&& lbType.equals(OBDefine.LBTYPE_SLB)) {
					if (OBUtility.isInteger(sp)) {
						obj.setSpNumber(sp);
					}
					obj.setAdcIndex(adcIndex);
					obj.setSrcIP(sourceIP);

					if (OBUtility.isInteger(sourcePort)) {
						obj.setSrcPort(sourcePort);
					}
					obj.setDstIP(matcher.group(1));
					obj.setDstPort(convertPort(matcher.group(2)));
					obj.setDamPort(matcher.group(3));
					obj.setRealIP(matcher.group(4));
					obj.setRealPort(convertPort(matcher.group(5)));
					// 6:protocol
					obj.setAgingTime(Integer.parseInt(matcher.group(7)));
				} else if ((matcher = exprSlbDstNotDamOrFilterWcr.matcher(dstPart)).matches() == true
						&& lbType.equals(OBDefine.LBTYPE_SLB)) {
					if (OBUtility.isInteger(sp)) {
						obj.setSpNumber(sp);
					}
					...
					obj.setDstPort(convertPort(matcher.group(2)));
					obj.setDamPort("");
					obj.setRealIP(matcher.group(3));
					obj.setRealPort(convertPort(matcher.group(4)));
					// 5:protocol
					obj.setAgingTime(Integer.parseInt(matcher.group(6)));
				} else if ((matcher = exprFilterAllowDenyNat.matcher(dstPart)).matches() == true
						&& lbType.equals(OBDefine.LBTYPE_FLB)) {
					if (OBUtility.isInteger(sp)) {
						obj.setSpNumber(sp);
					}
					obj.setAdcIndex(adcIndex);
					...
					obj.setDamPort("");
					obj.setRealIP("");
					obj.setRealPort(matcher.group(3)); // action = allow or deny or nat
					// 4:protocol
					obj.setAgingTime(Integer.parseInt(matcher.group(5)));
				} else if ((matcher = exprFilterWcr.matcher(dstPart)).matches() == true
						&& lbType.equals(OBDefine.LBTYPE_FLB)) {
					if (OBUtility.isInteger(sp)) {
						obj.setSpNumber(sp);
					}
				...
				retVal.add(obj);
				// 검색 결과수 제한이 있으면 그에 맞는 수의 로그만 내준다. 헤더,빈줄등을 고려해 20줄을 더 뽑았기 때문에 정확한 숫자 확인이 필요하다.
	...
	// --------------------------- parseSeesionDumpList() 테스트함수
//	private void zTestParseSeesionDumpList() {
//		String slbSession = "1,04: 172.172.2.110 43926, 192.168.100.51 ftp -> 7909 192.168.199.52 ftp age 6 v:1  E";
//		String slbSession2 = "1,04: 172.172.2.110 43926, 192.168.100.51 ftp -> 7909 192.168.199.52 ftp age 6 v:1  E";
//		String flbSessionWcr = "7,03: 70.70.70.100 51976, 168.126.63.1 dns -> 40.40.40.1 wcr age 2 f:1001  v:1  E";
//		String flbSessionDeny = "7,03: 70.70.70.100 1, 80.80.80.100 13701  DENY age 0 f:5  v:1  ";
//		String flbSessionAllow = "7,03: 70.70.70.100 2049, 70.70.70.150 62310 ALLOW age 0 f:70  v:1";
//
//		ArrayList<OBDtoFaultSessionInfo> result = null;
//		try {
//			result = parseSeesionDumpList(1, slbSession, -1, "SLB"); // 세션 결과수 제한 없음
//			System.out.println("result = " + result);
//			result = parseSeesionDumpList(1, slbSession2, -1, "SLB"); // 세션 결과수 제한 없음
//			System.out.println("result = " + result);
//			result = parseSeesionDumpList(1, flbSessionWcr, -1, "FLB"); // 세션 결과수 제한 없음
//			System.out.println("result = " + result);
//			result = parseSeesionDumpList(1, flbSessionDeny, -1, "FLB"); // 세션 결과수 제한 없음
//			System.out.println("result = " + result);
//			result = parseSeesionDumpList(1, flbSessionAllow, -1, "FLB"); // 세션 결과수 제한 없음
//			System.out.println("result = " + result);
//		} catch (OBException e) {
//			e.printStackTrace();
//		}
//	}
	...
	public ArrayList<OBDtoFaultSessionInfo> remanufactoringParsedSessionList(Integer adcIndex,
		...
		for (OBDtoSessionSearchOption option : seachKeyList) {
			...
			} else if (option.getType() == OBDtoSessionSearchOption.OPTION_TYPE_AGING_TIME) {
				agingTime = Integer.parseInt(option.getContent());
			}
		}
		...
		for (OBDtoFaultSessionInfo info : sessionList) {
			...
			if (realIp != null && realIp.isEmpty() == false) {
				if (realIp.equals(info.getRealIP()) == false)
					continue;
			}
			if (realPort != null) {
			...
			if (agingTime != null && agingTime != 0) {
				if (agingTime.equals(info.getAgingTime()) == false)
					continue;
			}
			retVal.add(info);
		}
	...
	private final static String SUFFIX_VSTAT_ENABLED = "vstat enabled";//
	private final static String SUFFIX_VSTAT_DISABLED = "vstat disabled";//

	public Integer parseVstatState(String input) throws OBException {
		int strIndex = 0;
		String lines[] = input.split("\n");
		for (String line : lines) {
			strIndex = line.indexOf(SUFFIX_VSTAT_ENABLED);
			if (strIndex >= 0) {
				return OBDefine.STATE_ENABLE;
			}

			strIndex = line.indexOf(SUFFIX_VSTAT_DISABLED);
			if (strIndex >= 0) {
				return OBDefine.STATE_DISABLE;
			}

		}
		throw new OBException(OBException.ERRCODE_SYSTEM_INVALIDDATA, "failed to parse vstat state");
	}

	private final static String SUFFIX_SYSLOG_ENABLED = "syslogging all features";//
	...
	// private final static String SUFFIX_SYSLOG_ENABLED = "syslogging all
	// features";//
	public ArrayList<String> parseSyslogHost(String input) throws OBException {
		...
		Matcher mc = pSyslog.matcher(input);
...
//		syslogList.add(trimString);
	...
	private final static String SUFFIX_SNMP_ENABLED = "v1/v2 access enabled";//
	private final static String SUFFIX_SNMP_DISABLED = "Current v1/v2 access disabled";
...
//	}
	...
	public String parseSnmpState(String input) throws OBException {
		String lines[] = input.split("\n");
		for (String line : lines) {
			if (line.indexOf(SUFFIX_SNMP_ENABLED) != -1 || line.indexOf(SUFFIX_SNMP_DISABLED) != -1) {
				return line;
			}
		}
		return "";
	...
	private final static String SUFFIX_SNMP_RCOMM = "Read community string: ";//
	...
	private final static String SUFFIX_SNMP_USM = "Current SNMPv3 USM user settings:";//
	private final static String SUFFIX_SNMP_LAST = "Current SNMPv3 vacmAccess settings:";

	public String parseSnmpUsm(String input) throws OBException {
		...
		for (String line : lines) {
			if (line.isEmpty())
				continue;
			int strIndex = line.indexOf(SUFFIX_SNMP_USM);
			if (isCheck == true) {
				if (line.equals(SUFFIX_SNMP_LAST)) {
					return result;
				}
