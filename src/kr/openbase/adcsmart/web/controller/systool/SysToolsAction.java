package kr.openbase.adcsmart.web.controller.systool;
...
import kr.openbase.adcsmart.web.facade.systool.SysToolsFacade;
...
public class SysToolsAction extends BaseAction
{
    private transient Logger log = LoggerFactory.getLogger(SysToolsAction.class);
    
	@Autowired
	private SysToolsFacade sysToolsFacade;
    ...
    // 화면 load
    public String loadSysViewList() throws OBException
    {        
        return SUCCESS;
    }
    
    // dump file create
    public String startDumpFile() throws OBException
    {
        try
        {
            sysToolsFacade.runDumpFile();
        }
        catch (OBException e)
        {
            throw e;
    ...
    // dump file check
    public String isDumpDownloadFileExist() throws OBException
    {
        try 
        {            
//          String fileName = "C:\\var\\sysrpt.tgz";
            String fileName = "/var/sysrpt.tgz";
    ...
    public String dumpFileCreate() throws OBException
    {
        try
        {
            sysToolsFacade.runDumpDownload();
        }
        catch (OBException e) 
        {
            throw e;
    ...
    // dump file download
    public String runDumpDownload() throws OBException
    {
        try
        {
//            sysToolsFacade.runDumpDownload();
...
/*        catch (OBException e)
	...
	public String portUsageLoadContent() throws OBException 
	{
		...
		try
		{
			contents = sysToolsFacade.portUsageGetContent();
		}
		catch(OBException e)
		{
			throw e;
	...
	public String portUsageDownloadCsv() throws OBException 
	{
		try 
		{
			String [] header = sysToolsFacade.portUsageMakeCsvHeader();
			ArrayList<String[]> data = sysToolsFacade.portUsageMakeCsvBody();
	...
	public String slbSessionLoadContent() throws OBException 
	{
		...
		try
		{
			...
			{
				contents = sysToolsFacade.slbSessionGetContent(adcIPAddress ,adcType , adcUserID, adcPasswd, connService, connPort, searchIPType, searchIPAddress);				
			}
			else
			{
	...
	public String unUsedSlbLoadContent() throws OBException 
	{
		...
		try
		{
			adcList = sysToolsFacade.getAdcsByAccountIndex(session.getAccountIndex());
			if(adcIndex==null)
				adcIndex=0;
			if(searchType==null)
				searchType=0;
			contents = sysToolsFacade.unUsedSlbGetContent(session.getAccountIndex(), adcIndex, searchType);
		}
		catch(OBException e)
		{
			throw e;
	...
	public String unUsedSlbDownloadCsv() throws OBException 
	{
		try 
		{
			String [] header = sysToolsFacade.unUsedSlbMakeCsvHeader(session.getAccountIndex(), adcIndex);
			ArrayList<String[]> data = sysToolsFacade.unUsedSlbMakeCsvBody(session.getAccountIndex(), adcIndex);
...
//        }
    ...
    public String loadResponseInfoList() throws Exception
    {
        try 
        {
            ...
            respGroupInfoList = sysToolsFacade.getRespInfoList(searchOption, orderType, orderDir);
    ...
    public String loadResponseTimeHistoryInfo() throws Exception
    {
        try
        {
            ...
            if (respIndex != null)
            {                
                ResponseTimeInfo = sysToolsFacade.getResponseTimeHistory(respIndex, searchOption);                
            }
            else
            {                
                ResponseTimeInfo = sysToolsFacade.getResponseTimeHistory(null, searchOption);             
            }
    ...
    public Boolean isExistRespIntervalCheck() throws Exception
    {
        ...
        try
        {
            ...
            if(sysToolsFacade.isExistRespIntervalCheck(respName))
            {
                isSuccessful = false;
                message = OBMessageWeb.getMessageWeb(OBMessageWeb.MSG_NAME_DUPLICATED);
                return false;
            }
            sysToolsFacade.addRespIntervalCheck(respGroupInfo);
...
//                return false;
//            }            
            sysToolsFacade.setRespIntervalCheck(respGroupInfo);
...
//                return SUCCESS;
//            }
            sysToolsFacade.delRespIntervalCheck(respInfoIndexes);
    ...
    public String loadRespInfoModifyContent() throws Exception
    {
        try 
        {
            respGroupInfo = sysToolsFacade.getRespInfo(respIndex);
    ...
    public SysToolsFacade getSysToolsFacade()
    {
        return sysToolsFacade;
    }

    public void setSysToolsFacade(SysToolsFacade sysToolsFacade)
    {
        this.sysToolsFacade = sysToolsFacade;
    }
    ...
    {
        return "SysToolsAction [sysToolsFacade=" + sysToolsFacade
                + ", FILE_NAME_PORT_USAGE_CSV=" + FILE_NAME_PORT_USAGE_CSV
                + ", FILE_NAME_UNUSED_SLB_CSV=" + FILE_NAME_UNUSED_SLB_CSV
                + ", FILE_NAME_USED_SLB_CSV=" + FILE_NAME_USED_SLB_CSV
