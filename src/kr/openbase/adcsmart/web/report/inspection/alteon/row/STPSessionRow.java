...
import kr.openbase.adcsmart.web.report.inspection.alteon.parser.STPUseStateParser;
...
public class STPSessionRow extends AbstractRow
{
    ...
    public STPSessionRow(final String swVersion, final AlteonDataHandler dataHandler)
    {
    ...
    public OBDtoInspectionReportRow getRow(final Column col) throws OBException
    {
        ...
        reportRowDto.setInspectionlist(OBMessages.getMessage(OBMessages.MSG_RPT_L2_STP_STATE));
        reportRowDto.setCheckMethod("/info/l2/stg");

        try
        {
            final String commandResult = dataHandler.getInfoStg();
            final List<String> parsedResult = new STPUseStateParser(swVersion).parse(commandResult);
    ...
    {
        for(final String str : parsedString)
        {
            final String regex = "\\d+\\(" + ON_KEYWORD + "\\)";
