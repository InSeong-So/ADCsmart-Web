...
public class SerialNumberParser extends AbstractAlteonParser {
	...
	public List<String> parse(String input) {
		...
		// 29.x 이상, 이하 동시 호환
		Matcher match = getMatcher("Serial Number\\s*:\\s*(.+)|Serial No:\\s+(.+)\\s+Rev:", input);

		if (match.find()) {
			String serialNumberStr = match.group(1);
			if (serialNumberStr != null) {
				macAddr.add(serialNumberStr.trim());
			}

			serialNumberStr = match.group(2);
			if (serialNumberStr != null) {
				if (serialNumberStr.equalsIgnoreCase(NOT_AVAILABLE)) {
					macAddr.add("-");
				} else {
					macAddr.add(serialNumberStr);
				}
			}
		}
