...
import kr.openbase.adcsmart.web.report.inspection.alteon.parser.InterfaceStateParser;
...
public class InterfaceStateRow extends AbstractRow
{
    ...
    public InterfaceStateRow(final String swVersion, final AlteonDataHandler dataHandler)
    {
    ...
    public OBDtoInspectionReportRow getRow(final Column col) throws OBException
    {
        ...
        reportRowDto.setInspectionlist(OBMessages.getMessage(OBMessages.MSG_RPT_L3_INTERFACE_STATE));
        reportRowDto.setCheckMethod("/info/l3/ip");

        try
        {
            final String commandResult = dataHandler.getInfoL3Ip();
            final List<String> parsedResult = new InterfaceStateParser(swVersion).parse(commandResult);
