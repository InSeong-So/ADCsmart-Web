...
public class LinkUpLogRow extends AbstractRow {
//    private final AlteonDataHandler dataHandler;
//    private final String            swVersion;
	private Integer adcIndex;

	public LinkUpLogRow(final String swVersion, final AlteonDataHandler dataHandler, final Integer adcIndex) {
...
//		this.dataHandler = dataHandler;
	...
	public OBDtoInspectionReportRow getRow(final Column col) throws OBException {
		...
		try {
			...
			reportRowDto.setInspectionlist(OBMessages.getMessage(OBMessages.MSG_RPT_L2_LINK_UPLOG_STATE));
			reportRowDto.setCheckMethod("/info/sys/log");
			reportRowDto.setChecklist(checkLinkUpLog());
	...
	private String checkLinkUpLog() throws Exception {
		String sqlText = "";
		...
		try {
			...
			sqlText = String.format(
					...
					adcIndex, OBParser.sqlString(OBDefine.LINK_DOWN_LOG.LINK_DOWN_ALTEON));

			ResultSet rs = db.executeQuery(sqlText);
			if (rs.next() == true) {
				retVal = "Link down on port";
				return retVal;
			}
