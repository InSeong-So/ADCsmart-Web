...
public class TemperatureLogRow extends AbstractRow {
//    private final AlteonDataHandler dataHandler;
//    private final String            swVersion;  
	private Integer adcIndex;

	public TemperatureLogRow(final String swVersion, final AlteonDataHandler dataHandler, final Integer adcIndex) {
...
//		this.dataHandler = dataHandler;
	...
	public OBDtoInspectionReportRow getRow(final Column col) throws OBException {
		...
		try {
			...
			reportRowDto.setInspectionlist(OBMessages.getMessage(OBMessages.MSG_RPT_BASIC_TEMPLOG_STATE));
			reportRowDto.setCheckMethod("/info/sys/log");
			reportRowDto.setChecklist(checkTemperatureLog());
	...
	private String checkTemperatureLog() throws OBException {
		String sqlText = "";
		...
		try {
			...
			sqlText = String.format(
					...
					adcIndex, OBParser.sqlString(OBDefine.TEMP_HIGH.TEMP_HIGH));

			ResultSet rs = db.executeQuery(sqlText);
			if (rs.next() == true) {
				retVal = "Temperature exceeds threshold";
				return retVal;
			}
