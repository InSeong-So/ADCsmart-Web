...
public class SLBSessionStatsParser extends AbstractAlteonParser {
	...
	public SLBSessionStatsParser(String swVersion) {
		super(swVersion);
	}

	@Override
	public List<String> parse(String input) {
		final List<String> slbSessionStats = new ArrayList<String>();

		Matcher match = getMatcher(SUFFIX_STATSLB_MAX_SESSION + "\\s*(\\d+)", input);
		if (match.find()) {
			if (match.groupCount() == MAX_BOUNDS) {
				slbSessionStats.add(match.group(1));
			}
		}
